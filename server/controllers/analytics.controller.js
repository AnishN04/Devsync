...
const getAnalytics = async (req, res, next) => {
    try {
        ...
        // 5. Sprint Burndown Simulation
        const burndownData = [
            { day: 'Day 1', ideal: totalTasks, actual: totalTasks },
            { day: 'Day 2', ideal: Math.round(totalTasks * 0.8), actual: totalTasks - Math.round(doneTasks * 0.1) },
            { day: 'Day 3', ideal: Math.round(totalTasks * 0.6), actual: totalTasks - Math.round(doneTasks * 0.3) },
            { day: 'Day 4', ideal: Math.round(totalTasks * 0.4), actual: totalTasks - Math.round(doneTasks * 0.6) },
            { day: 'Day 5', ideal: Math.round(totalTasks * 0.2), actual: totalTasks - Math.round(doneTasks * 0.8) },
            { day: 'Day 6', ideal: 0, actual: totalTasks - doneTasks },
        ];

        // 6. Cumulative Flow Simulation
        const cumulativeData = [
            { name: 'Mon', Done: Math.round(doneTasks * 0.2), 'In Progress': Math.round(totalTasks * 0.1), Todo: Math.round(totalTasks * 0.7) },
            { name: 'Tue', Done: Math.round(doneTasks * 0.4), 'In Progress': Math.round(totalTasks * 0.2), Todo: Math.round(totalTasks * 0.4) },
            { name: 'Wed', Done: doneTasks, 'In Progress': statusDataMap['In Progress'], Todo: statusDataMap['Todo'] },
        ];
